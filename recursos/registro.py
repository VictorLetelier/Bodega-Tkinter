...
def cargar_registros():
    ...
    for i, registro in enumerate(registros):
        ...
        registro_evento = registro[2]
        id_empleado = registro[3]
        id_producto = registro[4]

        # Crea un label para mostrar los datos del registro
        label = tk.Label(inner_frame, text=f"ID Registro: {id_registro}\nFecha Registro: {fecha_registro}\nRegistro Evento: {registro_evento}\nID Empleado: {id_empleado}\nID Producto: {id_producto}", font=("Arial", 12), anchor="w", justify="left", wraplength=350,width=50, bg="#464646",fg="white")
